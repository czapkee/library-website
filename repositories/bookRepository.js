...
class BookRepository {
    ...
    async getNewBooks(limit = 10) {
        const query = `
            SELECT
                ...
                c.id as category_id,
                (SELECT COUNT(*) FROM book_sources WHERE book_id = b.id) as sources_count,
                (SELECT COUNT(*) FROM book_sources WHERE book_id = b.id) > 0 as has_sources
            FROM books b
    ...
    async searchBooks(query, category = 'all', sort = 'newest', limit = 20) {
        ...
        console.log('Search Query with sorting:', searchQuery);
